...
int lnsocket_handshake_with_tor(struct lnsocket *ln, const char *host, const char *port)
{
	...
	u8 request[SIZE_OF_REQUEST] = (u8 []) { SOCKS_V5, 0x01, SOCKS_NOAUTH };
    ...
    if (write(ln->socket, buffer, sizeof(buffer)) != sizeof(buffer))
        return note_error(&ln->errs,"Failed writing buffer: %s",
            strerror(errno));

	// read completion
    u8 inbuffer[SIZE_OF_IPV4_RESPONSE + SIZE_OF_RESPONSE];
    if (!read_all(ln->socket, inbuffer, sizeof(inbuffer)))
        return note_error(&ln->errs,"Failed reading inbuffer: %s",
            strerror(errno));
	if (inbuffer[1] != '\0')
...
int lnsocket_connect(struct lnsocket *ln, const char *node_id, const char *host, const char *tor_proxy)
{
	int timeout = tor_proxy ? DEFAULT_TOR_TIMEOUT : DEFAULT_TIMEOUT;
	return lnsocket_connect_with(ln, node_id, host, tor_proxy, timeout);
}
